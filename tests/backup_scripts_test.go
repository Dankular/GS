...
func TestBackupScriptsSupportMandatoryAgeEncryptionAndRestore(t *testing.T) {
	...
	for _, required := range []string{`createdb -U "$database_user"`, `dropdb -U "$database_user"`, `psql -U "$database_user"`, "economy.wallet_accounts", "economy.ledger_entries", "reconciliation drift: 0"} {
		if !strings.Contains(string(restore), required) {
