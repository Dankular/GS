...
func TestComposeSeparatesNakamaDatabaseRole(t *testing.T) {
	...
	text := string(data)
	for _, required := range []string{"nakama-role-bootstrap:", "DO $$$$", "CREATE ROLE nakama", "REVOKE ALL ON SCHEMA platform, economy, progression, match, ops FROM nakama", "database.address nakama:${NAKAMA_DATABASE_PASSWORD}@postgres:5432/gameservice", "NAKAMA_DATABASE_PASSWORD: ${NAKAMA_DATABASE_PASSWORD:?set NAKAMA_DATABASE_PASSWORD}"} {
		if !strings.Contains(text, required) {
