...
func TestComposeSeparatesNakamaDatabaseRole(t *testing.T) {
	...
	text := string(data)
	for _, required := range []string{"nakama-role-bootstrap:", "CREATE ROLE nakama", "ALTER TABLE public.%I OWNER TO nakama", "ALTER SEQUENCE public.%I OWNER TO nakama", "\\gexec", "REVOKE ALL ON SCHEMA platform, economy, progression, match, ops FROM nakama", "database.address nakama:${NAKAMA_DATABASE_PASSWORD}@postgres:5432/gameservice", "NAKAMA_DATABASE_PASSWORD: ${NAKAMA_DATABASE_PASSWORD:?set NAKAMA_DATABASE_PASSWORD}"} {
		if !strings.Contains(text, required) {
